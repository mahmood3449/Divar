  ...
  <div class="container">
      <div class="modal">
            <div class="modal__header">
                {{ search }}
                <h1>انتخاب شهر</h1>
                <button v-on:click="sendShowData"><i class="fa-solid fa-xmark"></i></button>
            </div>
            <div class="modal__search"><input type="text" placeholder="جستجو در میان آگهی ها" v-model.trim="city" v-on:blur="searchCity"><i class="fa-solid fa-magnifying-glass"></i></div>
            <div class="modal__showcity">
                <div class="modal__showcity-city" v-on:click="setLocation(city.name)" v-for="city in cities" :key="city.id">{{ city.name }}</div>
            </div>
      </div>
  </div>
</template>
...
search: null
    ...
    methods:{
        ...
        searchCity(){
          this.cities = this.cities.filter((element)=>{
              return element.name === this.city;
            })
        } ,
        setLocation(city){
        this.$axios.
        get(`https://one-api.ir/divar/?token=343050:60d87b72a4b9e8.68277188&action=cities&q=${city}`)
        .then((data)=>{
            localStorage.setItem('cityfa' , city);
            localStorage.setItem('cityen' , data.data.result);
            this.sendShowData();
    ...
    .container{
        ...
        .modal{
          cursor: pointer;
            ...
            background: white;
            text-align: center;
            box-shadow: 0px 4px 5px rgb(240, 240, 240);
          input{
              width: 75%;
